...
export default function ChatPage() {
    ...
    const init = async () => {
      try {
        ...
        if (d.success && d.conversations) {
          ...
          const personal = d.conversations.find((c: any) => c.type === 'ai_personal');
          if (personal && !isMobile) {
            setActiveConv(personal);
            await loadMessages(personal.conv_id);
          }
  ...
  const openAgentChat = async (agent: any) => {
    try {
      const type = agent.is_personal ? 'ai_personal' : 'ai_agent';
      const r = await fetch('/api/chat/conversations', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ type, agentId:agent.agent_id }) });
      const d = await r.json();
      if (d.success) {
        await loadConversations();
        openConversation({ ...d.conversation, conv_id:d.conv_id, type, agent_id:agent.agent_id, agent_avatar:agent.avatar, agent_name:agent.name });
      }
    } catch {}
  };
          ...
          <div className={`${styles.left} ${isMobile && showMobileChat ? styles.mobileHidden : ''}`}>
            ...
            <div className={styles.convList}>
              {loading ? (
                ...
                <div style={{ padding:'20px', textAlign:'center', color:'#9CA3AF', fontSize:'0.875rem' }}>No conversations yet</div>
              ) : (
            ...
            {(tab === 'all' || tab === 'ai') && (() => {
              const convAgentIds = new Set(conversations.filter(c => c.type === 'ai_agent' && c.agent_id).map(c => c.agent_id));
              const roleAgents = agents.filter(a => !a.is_personal && convAgentIds.has(a.agent_id));
                  ...
                  <div className={styles.agentList}>
                    {roleAgents.map(agent => (
                      <div key={agent.agent_id} className={styles.agentItem} onClick={() => openAgentChat(agent)}>
                        ...
                        <div><div className={styles.agentName}>{agent.name}</div><div className={styles.agentRole}>{agent.role}</div></div>
                      </div>
                    ))}
                  </div>
                </div>
